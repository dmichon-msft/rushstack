...
describe(LookupByPath.prototype.findChildPathAndIndex.name, () => {
  ...
  it('returns the matching parent for multi-layer queries', () => {
    ...
    expect(tree.findChildPathAndIndex('foo/bar')).toEqual({ value: 1, index: 7 });
    expect(tree.findChildPathAndIndex('bar/baz')).toEqual({ value: 2, index: 7 });
    expect(tree.findChildPathAndIndex('baz/foo')).toEqual({ value: 3, index: 7 });
    expect(tree.findChildPathAndIndex('foo/foo')).toEqual(1);
  });
});
